...
def torque(la, F_I, F_1y, F_2y, F_3y, x1, x2, x3, xa, theta, P, zsc, ha):
    ...
    for i in x:
        y.append((zsc*F_1y*macaulay(i, x1, 0) + (ha/2)*F_I*np.cos(theta)*macaulay(i, xi1, 0) + zsc*F_I*np.sin(theta)*macaulay(i, xi1, 0) + zsc* F_2y*macaulay(i, x2, 0) - zsc*P*np.sin(theta)*macaulay(i, xi2, 0) - P*np.cos(theta)*(ha/2)*macaulay(i, x2, 0) + zsc*F_3y*macaulay(i, x3, 0) + AV3.DoubleIntegralZSC(i, zsc)))
...
def v_deflection(la, F_I, F_1y, F_2y, F_3y, x1, x2, x3, xa, theta, P, c1, c2,  E, I_zz):
    ...
    for i in x_v:
        y_v.append(k*(-(F_1y/6)*macaulay(i, x1, 3) - (F_I/6)*np.sin(theta)*macaulay(i, xi1, 3) - (F_2y/6)*macaulay(i, x2, 3) + (P/6)*np.sin(theta)*macaulay(i, xi2, 3) - (F_3y/6)*macaulay(i, x3, 3) + AV3.FiveIntegral(i)) + c1*i + c2) #AV3.FiveIntegral(i)
