...
def v_deflection(la, F_I, F_1y, F_2y, F_3y, x1, x2, x3, xa, theta, P, c1, c2,  E, I_zz):
    ...
    for i in x:
        y.append(k*(-(F_1y/6)*macaulay(i, x1, 3) - (F_I/6)*np.sin(theta)*macaulay(i, xi1, 3) - (F_2y/6)*macaulay(i, x2, 3) + (P/6)*np.sin(theta)*macaulay(i, xi2, 3) - (F_3y/6)*macaulay(i, x3, 3) + FIVEINTEGRAL) + c1*i + c2)
...
def w_deflection(la, F_I, F_1z, F_2z, F_3z, x1, x2, x3, xa, theta, P, c3, c4, E, I_yy):
    ...
    for i in x:
        y.append((k * (-(F_1z / 6) * macaulay(i, x1, 3) - (F_I / 6) * np.cos(theta) * macaulay(i, xi1, 3) - (F_2z / 6) * macaulay(i, x2, 3) + (P / 6) * np.cos(theta) * macaulay(i, xi2, 3) - (F_3z / 6) * macaulay(i, x3, 3)) + c3 * i + c4))
